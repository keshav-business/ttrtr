...
template = """You are a helpful AI assistant with access to knowledge about UBIK and subsidiaries. when talking about medicines for ex 2.5% gel then send response as 2 point 5% gel, dont use dot in this condition.
...
async def check_face_stream(face_cascade, timeout_seconds=300000000000000000000000000000000000000000000000000000):  # 5 minute timeout
    """Check RTSP stream continuously for faces using concurrent frame processing"""
    logger.info("Starting continuous face detection from stream")
    ...
    frame_batch_size = 5  # Number of frames to process concurrently
    batch_count = 0
    ...
    try:
        while (asyncio.get_event_loop().time() - start_time) < timeout_seconds:
            ...
            # Grab multiple frames at once
            for _ in range(frame_batch_size):
                # Clear buffer by grabbing frames
                for _ in range(2):  # Skip some frames to get fresher data
                    face_stream.grab()
            ...
            if not frames:
                logger.warning("No frames captured in current batch")
                await asyncio.sleep(0.1)  # Short sleep before next attempt
                continue
            ...
            tasks = [process_frame(frame, face_cascade) for frame in frames]
            results = await asyncio.gather(*tasks)
            
            # If any frame contains a face, return True
            if any(results):
                logger.info(f"Face detected in batch {batch_count}")
                return True
        ...
        logger.warning(f"Face detection timed out after {timeout_seconds} seconds")
        return False
...
async def get_ai_response(text: str, name: Optional[str] = None) -> str:
    ...
    try:
        # Extract contact information using GPT
        contact_prompt = (
        ...
        # Replace decimal points with commas in numbers
        # This regex looks for numbers with decimal points and replaces the point with a comma
        response = re.sub(r'(\d+)\.(\d+)', r'\1,\2', response)
        
        if name:
            response = f"{name}, {response}"
            
        # Replace any remaining periods with commas at the end of sentences
...
async def check_card_stream():
    ...
    Checks exactly 10 frames over 10 seconds."""
    ...
    try:
        for frame_count in range(10):  # Check exactly 10 frames
            logger.debug(f"Processing frame {frame_count + 1}/10")
            ...
            if not ret:
                logger.warning(f"Failed to read frame {frame_count + 1}")
                await asyncio.sleep(1)  # Wait 1 second before next frame
                continue

            logger.info(f"Successfully read frame {frame_count + 1} from card stream")

            try:
                # Convert frame to bytes
                success, encoded_frame = cv2.imencode('.jpg', frame)
                if not success:
                    logger.warning(f"Failed to encode frame {frame_count + 1}")
                    await asyncio.sleep(1)
                    continue
                
                # Process frame with Vision API
                text = await process_image_with_vision_api(encoded_frame.tobytes())
                logger.info(f"Card stream Vision API text from frame {frame_count + 1}: {text!r}")
                
                if text:
                    is_valid, contact_info = await validate_contact_info(text)
                    if is_valid:
                        logger.info(f"Valid contact info found in frame {frame_count + 1}: {contact_info}")
                        return contact_info
            except Exception as e:
                logger.error(f"Card processing error on frame {frame_count + 1}: {str(e)}")
            
            # Wait exactly 1 second before next frame
            await asyncio.sleep(1)
        
        logger.warning("Card stream check completed without finding valid contact info")
        return None
    finally:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def end_session():
        nonlocal face_task, face_detected
        # Clean up tasks
        ...
        try:
            ...
            if 1==1:
                try:
                    ...
                    if success:
                        logger.info(f"Meeting summary email sent")
                    else:
        ...
        # Start new face detection for next session
    ...
    try:
        ...
        while True:
            ...
            if current_time - last_message_time > 10000000000000000000000000000000:  # 10 seconds timeout
            ...
            try:
                ...
                last_message_time = asyncio.get_event_loop().time()  # Update last message time
                
                # Check for thank you and done
                if isinstance(data, str) and "thankyou" in data.lower() and "done" in data.lower():
                    logger.info("User indicated session completion")
                    await end_session()
                    break
